    ...
    $(document).ready(function() {
        ...
        $("#invoice-list-table").appTable({
            ...
            columns: [{
                ...
                // }
                { title: '<i data-feather="menu" class="icon-16"></i>', class: "text-center w80", visible: optionVisibility }

            ],
            printColumns: combineCustomFieldsColumns([1, 2, 3, 4, 7], '<?php echo $custom_field_headers; ?>'),
            xlsColumns: combineCustomFieldsColumns([1, 2, 3, 4, 7], '<?php echo $custom_field_headers; ?>'),
            summation: [{
                    column: 5,
                    dataType: 'currency',
...
<style>
  .actions-inline{display:inline-flex;align-items:center;gap:8px;justify-content:center}
  .action-chip{
  ...
  .action-chip svg{width:16px;height:16px;stroke:#334155}
  .action-chip:hover{background:#eef2ff;border-color:#c7d2fe;transform:translateY(-1px)}
  .action-chip:hover svg{stroke:#1d4ed8}

  /* Delete tone */
  .action-chip--danger svg{stroke:#b91c1c}
  .action-chip--danger:hover{background:#fef2f2;border-color:#fecaca}
  .action-chip--danger:hover svg{stroke:#dc2626}
</style>
