<?php echo form_open(get_uri("invoice_supplier_payments/save_payment"), array("id" => "invoice-payment-form", "class" => "general-form", "role" => "form")); ?>
<div class="modal-body clearfix">
    <div class="container-fluid">
        ...
        <?php } else { ?>
            <div class="form-group" id="invoice_selection_section" style="display: none;">
                <div class="row">
                    <label for="invoice_id" class=" col-md-3"><?php echo app_lang('invoice'); ?></label>
                    <div class="col-md-9">
                        <?php
                        echo form_dropdown("invoice_id", array("" => "-"), "", "class='select2 validate-hidden' id='invoice_id' data-rule-required='true' data-msg-required='" . app_lang('field_required') . "' ");
                        ?>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#invoice-payment-form #invoice_payment_method_id").select2({
            data: <?php echo $payment_methods_dropdown; ?>
        });



        // get due balance of selected supplier
        $("#supplier_id").on("change", function() {
            var supplier_id = $(this).val();
            if (supplier_id) {
                $.ajax({
                    url: "<?php echo get_uri("invoice_supplier_payments/get_supplier_payment_amount_suggestion"); ?>/" + supplier_id,
                    cache: false,
                    type: 'POST',
                    dataType: "json",
                    success: function(response) {
                        if (response && response.success) {
                            $("#invoice_payment_amount").val(response.supplier_total_summary.balance_due);
                        }
                    }
                });
            }
