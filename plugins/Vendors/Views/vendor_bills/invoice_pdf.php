<!DOCTYPE html>
<html lang="en">
<?php $color = get_setting("invoice_color") ?: "#2AA384";
$colspan = 3;
$show_taxable = false;
...
<body>
    <table style="width: 100%; margin-bottom: 10px;">
        ...
        <tr>
            <td style="width: 12%;">
                ...
                <img id="favicon-preview" src="<?php echo get_favicon_url(); ?>" alt="..." style="width: 100px" />
            </td>
            <td style="width: 100%;">
    ...
    <table style="width: 100%; margin-bottom: 30px;">
        <tr>
            <td style="width: 60%; vertical-align: top;">
                <h5 class="invoice-title" style=" color:<?php echo $color ?>"><strong>Invoice To:</strong></h5>
                <label><strong><?php echo $client_info->company_name; ?></strong></h6>
                    <label><?php echo $client_info->address; ?></label>
    ...
    <table>
        ...
        <tr style="background-color:<?php echo $color ?>;">
            <th style="width: <?php echo $show_taxable ? '40%' : '50%'; ?>;">Description</th>
            ...
            foreach ($invoice_items as $item) { ?>
                ...
                <?php if ($item->is_section) {
                    if ($current_section !== null) {
                        ...
                        <tr style="background-color:<?php echo get_setting("invoice_item_list_background") ?>;">
                            <td style="width: 84%;"></td>
                            <?php if ($show_taxable) { ?>
                                <td style="width: 2%;"></td>
                            <?php } ?>
                            <td style=" width:16% ;text-align: left;font-weight: bold;"><?php echo to_currency($section_total, $invoice_total_summary->currency_symbol); ?></td>
                        </tr>
                    <?php
                    ...
                    <tr style="font-weight: bold; background-color: <?php echo get_setting("section_color"); ?>;">
                ...
                ?>
                    ...
                    <tr style="background-color:<?php echo get_setting("invoice_item_list_background") ?>;">
                        ...
                        <td style="width: <?php echo $show_taxable ? '40%' : '50%'; ?>;"><?php echo $item->title; ?><br />
                        ...
                        <?php } ?>
                        <td style="width:16%"><?php echo to_currency($item->total, $item->currency_symbol); ?></td>
                    </tr>
        ...
        <tfoot>
            <tr>
                <td style="width: <?php echo $show_taxable ? '58%' : '65%'; ?>;text-align: left;"></td>
                <?php if ($show_taxable) { ?>
                    <td></td>
                <?php } ?>
                <td style="width: 17%;text-align: left;">Sub total</td>
                <td style="text-align: left; width: 20%; border: 1px solid #fff;"><?php echo to_currency($invoice_total_summary->invoice_subtotal, $invoice_total_summary->currency_symbol); ?>
                </td>
            </tr>
            ...
            if ($invoice_total_summary->discount_total && $invoice_total_summary->discount_type == "before_tax") { ?>
                <tr>
                    ...
                    <?php if ($show_taxable) { ?>
                        <td></td>
                    <?php } ?>
                    <td style="width: 17%;text-align: left;"><?php echo app_lang("discount") ?></td>
            ...
            <?php if ($invoice_total_summary->tax) { ?>
                <tr class="total">
                    <td style="width: <?php echo $show_taxable ? '58%' : '65%'; ?>;"></td>
                    <?php if ($show_taxable) { ?>
                        <td></td>
                    <?php } ?>
                    <td style="width: 17%;"><?php echo $invoice_total_summary->tax_name; ?></td>
            ...
            <tr class="total">
                ...
                <?php if ($show_taxable) { ?>
                    <td></td>
                <?php } ?>
                <td style="width: 17%; text-align: left;"><strong>Total</strong></td>
            ...
            <?php }
            if ($invoice_total_summary->Advance && empty($invoice_total_summary->total_paid)) {
                $advance_percentage = $invoice_total_summary->Advance;
                $advance_amount = $invoice_total_summary->invoice_total * ($advance_percentage / 100);
            ?>
                <tr>
                    <td style="width: <?php echo $show_taxable ? '58%' : '65%'; ?>;"></td>
                    <?php if ($show_taxable) { ?><td></td><?php } ?>
                    <td style="width: 17%; text-align: left;">Advance <?php echo $advance_percentage; ?>%</td>
                    <td style="text-align: left; width: 20%;"><?php echo to_currency($advance_amount, $invoice_total_summary->currency_symbol); ?></td>
                </tr>
            <?php } ?>
            <!-- // if ($invoice_total_summary->Advance) { ?> -->
            <!-- <tr>
                    <td style="width: <?php echo $show_taxable ? '58%' : '65%'; ?>;"></td>
                    <? php // if ($show_taxable) { 
                    ?>
                    ...
                    // $advance_amount = $invoice_total_summary->invoice_total * ($advance_percentage / 100);
                    ?>
                    <td style="width: 17%; text-align: left;">Advance <?php // echo $advance_percentage; 
                                                                        ?>%</td>
                    <td style="text-align: left; width: 20%;"><?php // echo to_currency($advance_amount, $invoice_total_summary->currency_symbol); 
                                                                ?></td>
                </tr> -->
            <?php //} 
            ?>
    ...
    <table>
        <tr>
            <td style="width:60%;">
                <h6> <?php echo get_setting("invoice_footer"); ?> </h6>

            </td>
            <td style="width: 40%; text-align: center;">

  <!-- signature image(s) -->
  <div class="signature-wrapper" style="display:inline-block; margin-bottom:14px;">
    <?= $signature ?>
  </div>

  <!-- name & title -->
  <div class="signature-block" style="margin-top: 6px;">
    <?php if (!empty($finance_manager_user_info->first_name)) { ?>
      <h5 class="signature-name" style="color: <?= $color ?>; margin:0;">
        <?= $finance_manager_user_info->first_name . ' ' . $finance_manager_user_info->last_name; ?>
      </h5>
    <?php } ?>
    <?php if (!empty($finance_manager_job_info->job_title_en)) { ?>
      <h5 class="signature-title" style="margin:4px 0 0 0;">
        <?= $finance_manager_user_info->job_title; ?>
      </h5>
    <?php } ?>
  </div>
</td>
