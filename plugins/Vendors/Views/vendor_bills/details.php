            ...
            <div style="max-width: 1000px; margin: auto;">
                <div class="clearfix p20">
                    ...
                    if ($invoice_style === "style_3") {
                        echo view('Sales_and_crm\Views/invoices/invoice_parts/header_style_3.php', $data);
                    } else if ($invoice_style === "style_2") {
                        echo view('Sales_and_crm\Views/invoices/invoice_parts/header_style_2.php', $data);
                    } else {
                        echo view('Sales_and_crm\Views/invoices/invoice_parts/header_style_1_view.php', $data);
                    }
                    ?>
                </div>
                ...
                <div class="clearfix">
                    <?php if ($can_add_new_invoices) { ?>
                        ...
                        <div class="float-start mt20 ml15">
                            <?php echo modal_anchor(get_uri("invoices/section_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . ('add section'), array("class" => "btn btn-primary text-white add-item-btn", "title" => ('add section'), "data-post-invoice_id" => $invoice_info->id)); ?>
                        </div>
                    <?php } ?>
    ...
    $(document).ready(function() {
        ...
        $("#invoice-item-table").appTable({
            ...
            rowCallback: function(nRow, aData) {
                var column = $("#invoice-item-table").DataTable().column(5);
                ...
                <?php if ($can_edit_invoices) { ?>
                    ...
                    Sortable.create($selector[0], {
                        ...
                        onUpdate: function(e) {
                            ...
                            // Build "type-id-index" CSV from current DOM order
                            var data = "";
                            $.each($selector.find(".item-row"), function(index, ele) {
                                if (data) data += ",";
                                var id = $(ele).attr("data-id");
                            ...
                            $.ajax({
                                ...
                                data: {
                                    sort_values: data,
                                    invoice_id: "<?php echo $invoice_info->id; ?>" // optional but safer
                                },
                                success: function() {
                                    const dt = $("#invoice-item-table").DataTable();
                                    // Pull fresh rows (with updated 'sort' numbers) but keep current page
                                    dt.ajax.reload(function() {
                                        // Keep 1 at the top; newest (max sort) stays at bottom
