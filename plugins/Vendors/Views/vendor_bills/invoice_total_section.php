<table id="invoice-item-table" class="table display dataTable text-right strong table-responsive no-body-top-bottom-border">
    ...
    ?>
     <tr>
        ...
        <?php if ($invoice_total_summary->Advance) { ?>
        <tr>
        <?php   $advance_percentage=$invoice_total_summary->Advance;
                $advance_amount = $invoice_total_summary->invoice_total * ($advance_percentage / 100);
        ?>
            <td >Advance <?php echo $advance_percentage; ?>%</td>
            <td><?php echo to_currency($advance_amount, $invoice_total_summary->currency_symbol); ?></td>
            <?php if ($can_edit_invoices) { ?>
                <td></td>
            <?php } ?>
        </tr>
