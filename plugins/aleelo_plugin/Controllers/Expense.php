...
class Expense extends Security_Controller_Plugin {
    ...
    function yearly() {
        return $this->template->view("aleelo_plugin\Views/expenses/yearly_expenses");
    }
    ...
    function custom() {
        return $this->template->view("aleelo_plugin\Views/expenses/custom_expenses");
    }
    ...
    protected function can_view_all_expense() {
        if ($this->login_user->company_access === "all" || (get_array_value($this->login_user->permissions, "expense") === "all")) {
            return 0;
        }
else  if ($this->login_user->company_access === "all" || (get_array_value($this->login_user->permissions, "expense") === "own_company")) {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if ($data->recurring_expense_id) {
            ...
            $description .= modal_anchor(get_uri("expense/expense_details"), app_lang("original_expense"), array("title" => app_lang("expense_details"), "data-post-id" => $data->recurring_expense_id));
        }
        ...
        if ($data->files) {
            ...
            if (count($files)) {
                foreach ($files as $key => $value) {
                    ...
                    $file_download_link = anchor(get_uri("expense/download_files/" . $data->id), "<i data-feather='download'></i>", array("title" => app_lang("download")));
                    $files_link .= js_anchor("<i data-feather='$link'></i>", array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "class" => "float-start mr10", "title" => remove_file_prefix($file_name), "data-url" => get_uri("expense/file_preview/" . $data->id . "/" . $key)));
                }
            }
        }
    ...
    function import_expenses_modal_form() {
        return $this->template->view("aleelo_plugin\Views/expenses/import_expenses_modal_form");
    }
