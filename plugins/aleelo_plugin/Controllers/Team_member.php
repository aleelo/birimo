...
class Team_member extends Security_Controller_Plugin {
    ...
    function modal_form() {
        ...
        return $this->template->view('aleelo_plugin\Views/team_members/modal_form', $view_data);
    }
    ...
    function invitation_modal() {
        ...
        return $this->template->view('aleelo_plugin\Views/team_members/invitation_modal', $view_data);
    }
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if ($this->_can_delete_team_member($data)) {
            $delete_link = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_team_member'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("team_member/delete"), "data-action" => "delete-confirmation"));
        }
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            if ($user_info) {
                ...
                return $this->template->rander("aleelo_plugin\Views/team_members/view", $view_data);
            } else {
                show_404();
            }
        } else {
...
if (!$signature_path) {
    ...
    function projects_info($user_id) {
        if ($user_id) {
            ...
            return $this->template->view("aleelo_plugin\Views/team_members/projects_info", $view_data);
        }
    }
    ...
    function attendance_info($user_id) {
        if ($user_id) {
            validate_numeric_value($user_id);
            $view_data['user_id'] = $user_id;
            return $this->template->view("aleelo_plugin\Views/team_members/attendance_info", $view_data);
        }
    }
    ...
    function leave_info($applicant_id) {
        if ($applicant_id) {
            ...
            return $this->template->view("aleelo_plugin\Views/team_members/leave_info", $view_data);
        }
    }
    ...
    function expense_info($user_id) {
        ...
        return $this->template->view("aleelo_plugin\Views/team_members/expenses", $view_data);
    }
    ...
    function files($user_id) {
        ...
        $view_data['user_id'] = $user_id;
        return $this->template->view("aleelo_plugin\Views/team_members/files/index", $view_data);
    }
    ...
    function file_modal_form() {
        ...
        $view_data['user_id'] = $user_id;
        return $this->template->view('aleelo_plugin\Views/team_members/files/modal_form', $view_data);
    }
    ...
    private function _make_file_row($data) {
        ...
        $description = "<div class='float-start'>" .
            js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("team_member/view_file/" . $data->id), "class" => "text-break-space"));
        ...
        $options = anchor(get_uri("team_member/download_file/" . $data->id), "<i data-feather='download-cloud' class='icon-16'></i>", array("title" => app_lang("download")));

        if ($this->login_user->is_admin || $data->uploaded_by == $this->login_user->id) {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("team_member/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function view_file($file_id = 0) {
        ...
        if ($file_info) {
            ...
            return $this->template->view("aleelo_plugin\Views/team_members/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function notes($user_id) {
        ...
        if ($user_id) {
            ...
            return $this->template->view("aleelo_plugin\Views/team_members/notes/index", $view_data);
        }
    }
