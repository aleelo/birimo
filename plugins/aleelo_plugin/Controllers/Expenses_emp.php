...
class Expenses_emp extends Security_Controller_Plugin
{
    ...
    function index()
    {
        ...
        return $this->template->rander("aleelo_plugin\Views/expenses_emp/index", $view_data);
    }
    ...
    function summary()
    {
        ...
        return $this->template->rander("aleelo_plugin\Views/expenses_emp/reports/expenses_summary");
    }

    //load the recurring view of expense list 
    function recurring()
    {
        return $this->template->view("aleelo_plugin\Views/expenses_emp/recurring_expenses_list");
    }
    ...
    function modal_form()
    {
        ...
        $model_info = $this->Expenses_emp_model->get_one($id);
        ...
        if ($project_info) {
            ...
            $view_data['clients_dropdown'] = $this->get_clients_dropdown($model_info->project_id, true);
        }
        ...
        $view_data['user_id'] = $model_info->user_id;
        ...
        return $this->template->view('aleelo_plugin\Views/expenses_emp/modal_form', $view_data);
    }
    ...
    function get_projects_dropdown($client_id = 0, $return_as_list_data = false)
    {
        ...
        // optional: validate_numeric_value($client_id); // haddii la rabin in uu ku xayiro 0

        $options = array();

        // Haddii client_id jiro, ku dar filter, haddii kale, iska dhaaf
    ...
    function get_clients_dropdown($project_id = 0, $return_as_list_data = false)
    {
        if (!$project_id) {
        ...
        $filter = array("is_lead" => 0); // All clients

        if ($project_id) {
            $project_info = $this->Projects_model->get_one($project_id);
            if ($project_info->client_id) {
                $filter["id"] = $project_info->client_id;
            }
        }

        $clients_dropdown = $this->Clients_model->get_id_and_text_dropdown(array("company_name"), $filter, "-");
 ...
 public function save_payment() {
    ...
    if ($save_id) {
        ...
        // Get expense info
        $expense = $this->Expenses_emp_model->get_one($expense_id);

        // Compare total paid vs expense amount
        if ($total_paid >= $expense->amount) {
            $this->Expenses_emp_model->ci_save(["status" => "paid"], $expense_id);
        } elseif ($total_paid > 0) {
            $this->Expenses_emp_model->ci_save(["status" => "partially_paid"], $expense_id);
        }
   ...
   public function save()
{
    ...
    $expense_info = $this->Expenses_emp_model->get_one($id);
    ...
    $save_id = $this->Expenses_emp_model->ci_save($data, $id);
    ...
    function delete()
    {
        ...
        if ($this->Expenses_emp_model->delete($id)) {
            //delete the files
    ...
    function list_data($recurring = false)
    {
        ...
        $list_data = $this->Expenses_emp_model->get_details($options)->getResult();
    ...
    private function _row_data($id)
    {
        ...
        $data = $this->Expenses_emp_model->get_details($options)->getRow();
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        if ($data->recurring_expense_id) {
            ...
            $description .= modal_anchor(get_uri("expenses_emp/expense_details"), app_lang("original_expense"), array("title" => app_lang("expense_details"), "data-post-id" => $data->recurring_expense_id));
        }
        ...
        if ($data->files) {
            ...
            if (count($files)) {
                foreach ($files as $key => $value) {
                    ...
                    $file_download_link = anchor(get_uri("expenses_emp/download_files/" . $data->id), "<i data-feather='download'></i>", array("title" => app_lang("download")));
                    $files_link .= js_anchor("<i data-feather='$link'></i>", array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "class" => "float-start mr10", "title" => remove_file_prefix($file_name), "data-url" => get_uri("expenses_emp/file_preview/" . $data->id . "/" . $key)));
                }
            }
        }
        ...
        $total_amount_numeric = $data->amount + $tax + $tax2;
        $total_amount = to_currency($total_amount_numeric);
        $due = to_currency($total_amount_numeric - $data->expense_paid); // ✅ Now subtraction is numeric
        
        // $due = to_currency($total_amount - $data->expense_paid);

        $status = $this->_get_expense_status_label($data);

        $row_data = array(
            $data->expense_date,
            modal_anchor(get_uri("expenses_emp/expense_details"), format_to_date($data->expense_date, false), array("title" => app_lang("expense_details"), "data-post-id" => $data->id, "data-modal-lg" => "1")),
            $data->category_title,
            $data->title,
            $description,
        ...
        $row_data[] = modal_anchor(get_uri("expense_payments_emp/modal_form"), "<i data-feather='external-link' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('acc_convert'), "data-post-expense_id" => $data->id, "data-post-type" => 'expense', "data-post-form_type" => 'expense_form'))
            . modal_anchor(get_uri("expenses_emp/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_expense'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_expense'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("expenses_emp/delete"), "data-action" => "delete-confirmation"));

        return $row_data;
    }

    private function _get_expense_status_label($data, $return_html = true, $extra_classes = "")
    {
        return get_expense_status_label($data, $return_html, $extra_classes);
    }
    ...
    function file_preview($id = "", $key = "")
    {
        ...
        if ($id) {
            ...
            return $this->template->view("aleelo_plugin\Views/expenses_emp/file_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    {
        return $this->template->view("aleelo_plugin\Views/expenses_emp/reports/yearly_chart");
    }
    ...
    function yearly_chart_data()
    {
        ...
        if ($year) {
            $expenses = $this->Expenses_emp_model->get_yearly_expenses_chart($year, 0, $this->show_own_expenses_only_user_id());
            $values = array();
    ...
    function income_vs_expenses()
    {
        ...
        return $this->template->rander("aleelo_plugin\Views/expenses_emp/reports/income_vs_expenses_chart", $view_data);
    }
    ...
    function income_vs_expenses_chart_data()
    {
        ...
        if ($year) {
            $expenses_data = $this->Expenses_emp_model->get_yearly_expenses_chart($year, $project_id, $this->show_own_expenses_only_user_id());
    ...
    {
        $view_data["projects_dropdown"] = $this->_get_projects_dropdown_for_income_and_expenses();
        return $this->template->view("aleelo_plugin\Views/expenses_emp/reports/income_vs_expenses_summary", $view_data);
    }
    ...
    function income_vs_expenses_summary_list_data()
    {
        ...
        if ($year) {
            $expenses_data = $this->Expenses_emp_model->get_yearly_expenses_chart($year[0], $project_id, $this->show_own_expenses_only_user_id());
    ...
    function expense_list_data_of_project($project_id)
    {
        $this->access_only_team_members();
        validate_numeric_value($project_id);
    ...
    function expense_list_data_of_user($user_id)
    {
        $this->access_only_team_members();
    ...
    function expense_details()
    {
        ...
        $info = $this->Expenses_emp_model->get_details($options)->getRow();
        if (!$info) {
            show_404();
        }
        ...
        return $this->template->view("aleelo_plugin\Views/expenses_emp/expense_details", $view_data);
    }
    ...
    function summary_list_data()
    {
        ...
        $list_data = $this->Expenses_emp_model->get_summary_details($options)->getResult();
    ...
    function download_files($id)
    {
        ...
        $files = $this->Expenses_emp_model->get_one($id)->files;
    ...
    private function _save_a_row_of_excel_data($row_data)
    {
        ...
        //save expense data
        $saved_id = $this->Expenses_emp_model->ci_save($expense_data);
        if (!$saved_id) {
            return false;
        }
    ...
    function tasks($expense_id)
    {
        ...
        return $this->template->view("aleelo_plugin\Views/expenses_emp/tasks/index", $view_data);
    }

    //load the expenses monthly summary view
    function monthly_summary()
    {
        return $this->template->view("aleelo_plugin\Views/expenses_emp/reports/monthly_summary");
    }

    //load the expenses custom summary view
    function custom_summary()
    {
        return $this->template->view("aleelo_plugin\Views/expenses_emp/reports/custom_summary");
    }

    //load the expenses category chart view
    function category_chart()
    {
        return $this->template->view("aleelo_plugin\Views/expenses_emp/reports/category_chart_container");
    }
    ...
    function category_chart_view()
    {
        ...
        $categories_data = $this->Expenses_emp_model->get_summary_details($options)->getResult();
        ...
        return $this->template->view("aleelo_plugin\Views/expenses_emp/reports/category_chart", $view_data);
    }
}

/* End of file expenses.php */
/* Location: ./app/controllers/expenses_emp.php */