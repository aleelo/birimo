...
use aleelo_plugin\Models\Expense_payments_model;
 use Sales_and_crm\Models\Expenses_model;

class Expense_payments extends Security_Controller_Plugin
{
    function __construct()
    {
        parent::__construct();
        // $this->access_only_allowed_members();
        $this->Expense_payments_model = model("Expense_payments_model");
        $this->Expenses_model = model("Expenses_model");
    }
    ...
    public function index($expense_id = 0)
{
    ...
    $expense_model = new Expense_payments_model(); // Haddii loo baahdo
 ...
 return $this->template->rander("aleelo_plugin\Views/expenses/expense_payments/index", $view_data);
...
$view_data["amount"] = $amount;
 ...
 return $this->template->view("aleelo_plugin\Views/expenses/expense_payments/modal_form", $view_data);
    ...
    private function _make_row($data)
{
    return [
        ...
        modal_anchor(get_uri("expense_payments/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_payment'), "data-post-id" => $data->id, "data-post-invoice_id" => $data->expense_id)).
        js_anchor("<i data-feather='x' class='icon-16'></i>", ["class" => "btn btn-danger btn-sm", "title" => app_lang("delete"), "data-id" => $data->id, "data-action-url" => get_uri("expense_payments/delete/" . $data->id), "data-action" => "delete"])
    ];
}
