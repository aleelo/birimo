    ...
    <div class="clearfix">
        <ul id="client-files-tabs" data-bs-toggle="ajax-tab" class="nav nav-tabs bg-white title rounded-top-0" role="tablist">
            ...
            <?php if (get_setting("module_file_manager") == "1") { ?>
                <li><a role="presentation" data-bs-toggle="tab" href="<?php echo_uri("client/explore/" . $folder_id . "/1/" . $view_from . "/" . $client_id); ?>" data-bs-target="#file-grid-tab"><?php echo app_lang('folders'); ?></a></li>
            <?php } ?>
            <div class="tab-title clearfix no-border">
                <div class="title-button-group">
                    <?php
                    if ($login_user->user_type == "staff" || ($login_user->user_type == "client" && get_setting("client_can_add_files"))) {
                        echo modal_anchor(get_uri("client/file_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_files'), array("class" => "btn btn-default", "id" => "general-add-files-button", "title" => app_lang('add_files'), "data-post-client_id" => $client_id));
                    }
                    ?>
                </div>
            </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#client-file-table").appTable({
            source: '<?php echo_uri("client/files_list_data/" . $client_id) ?>',
            order: [
                [0, "desc"]
            ],
            columns: [{
