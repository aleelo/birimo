...
<div class="modal-body">
    ...
    <div class="form-group">
        <div class="row">
            ...
            <div class="col-md-9">
                <?php
                echo form_dropdown("expense_id",$expenses_dropdown, $model_info->expense_id ?? "", "class='select2 form-control validate-hidden' id='expense_id' data-rule-required='true' data-msg-required='" . app_lang('field_required') . "'");
                ?>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function () {
        $("#expense-payment-form").appForm({
            onSuccess: function (result) {
                $("#expense-payment-table").appTable({ reload: true });
            }
        });

        setDatePicker("#payment_date");

        // Init all select2 fields
        $("#vendor_id, #category_id, #expense_id, #payment_method,#deposit_to").select2();

        // Populate initial expense options (if any)
        let selectedExpenseId = "<?php echo $model_info->expense_id ?? ''; ?>";
        let initialExpenses = <?php echo json_encode($expenses_dropdown); ?>;



        $("#expense_id").select2().on("change", function () {
        ...
        $("#category_id").on("change", function () {
            ...
            if (vendor_id) {
                loadExpenses(vendor_id, category_id);
            }
        });
    });
</script>
