...
foreach ($comments as $comment) {
    ...
    $pin_status = "";
    $unpin_status = "";
    $file_download_url = "project/download_comment_files";
    if ($comment->file_id) {
        $type = "file";
                    ...
                    <div class="mb5">
                        ...
                        <span class="float-end comment-like-top">
                            ...
                            if (isset($comment->like_status)) {
                                ...
                                echo ajax_anchor(get_uri("project/like_comment/" . $comment->id), "<i data-feather='$like_icon' class='icon-14 $like_icon_fill'></i> " . app_lang('like') . " ", array("class" => "mr5 like-button", "data-real-target" => "#comment-like-container-$comment->id"));
                            }
                            ?>
                        </span>
                        ...
                        <div class="mb15 clearfix">
                            <?php if (isset($comment->like_status)) { ?>
                                <span id="comment-like-container-<?php echo $comment->id; ?>">
                                    <?php echo view("aleelo_plugin\Views/projects/comments/like_comment", array("comment" => $comment)); ?>
                                </span>
                            <?php } ?>
                            ...
                            if ($type === "project" || $type === "customer_feedback") {
                                $can_reply = true;
                                echo ajax_anchor(get_uri("project/comment_reply_form/" . $comment->id . "/" . $type . "/" . $type_id), "<i data-feather='corner-up-left' class='icon-16'></i> " . app_lang('reply'), array("data-real-target" => "#reply-form-container-" . $comment->id));
                            }
                            ?>
                            <?php
                            $reply_caption = "";
...
<script>
    $(document).ready(function () {
        ...
        $(".pin-comment-button").click(function () {
            ...
            appLoader.show();
            $.ajax({
                url: "<?php echo get_uri("project/pin_comment/"); ?>/" + comment_id,
                type: 'POST',
                dataType: "json",
