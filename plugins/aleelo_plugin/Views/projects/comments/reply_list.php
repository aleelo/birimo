                ...
                <?php if ($login_user->is_admin || $reply->created_by == $login_user->id) { ?>
                    <span class="float-end dropdown reply-dropdown">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <li role="presentation"><?php echo ajax_anchor(get_uri("project/delete_comment/$reply->id"), "<i data-feather='x' class='icon-16'></i> " . app_lang('delete'), array("class" => "dropdown-item", "title" => app_lang('delete'), "data-fade-out-on-success" => "#prject-comment-reply-container-$reply->id")); ?> </li>
                        </ul>
                    </span>
                <?php } ?>
            ...
            <div class="comment-image-box clearfix">
                ...
                if ($reply->files) {
                    ...
                    if ($total_files) {
                        ...
                        echo anchor(get_uri("project/download_comment_files/" . $reply->id), $download_caption, array("class" => "float-end", "title" => $download_caption));
                    }
                }
                ?>
            </div>
