...
class Clients_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($department_id) {
            $where .= " AND $clients_table.company_id=$department_id";
        }
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $clients_table.*, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS primary_contact, $company_table.name AS companies  , $users_table.id AS primary_contact_id, $users_table.image AS contact_avatar,  project_table.total_projects, IFNULL(invoice_details.payment_received,0) AS payment_received $select_custom_fieds,
        ...
        LEFT JOIN (SELECT client_id, SUM(payments_table.payment_received) as payment_received, SUM($invoices_table.invoice_total) AS invoice_value FROM $invoices_table
                   LEFT JOIN (SELECT invoice_id, SUM(amount) AS payment_received FROM $invoice_payments_table WHERE deleted=0 AND supplier_id=0 GROUP BY invoice_id) AS payments_table ON payments_table.invoice_id=$invoices_table.id AND $invoices_table.deleted=0 AND $invoices_table.status='not_paid'
        ...
        LEFT JOIN $lead_status_table ON $clients_table.lead_status_id = $lead_status_table.id 
               LEFT JOIN $company_table ON $company_table.id = $clients_table.company_id AND $company_table.deleted=0  
