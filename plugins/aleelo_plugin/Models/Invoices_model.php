...
class Invoices_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        LEFT JOIN (SELECT invoice_id, SUM(amount) AS payment_received FROM $invoice_payments_table WHERE deleted=0 AND supplier_id=0 GROUP BY invoice_id) AS payments_table ON payments_table.invoice_id = $invoices_table.id 
...
function get_invoice_total_summary($invoice_id) {
    ...
    //     $payment_sql = "SELECT SUM($invoice_payments_table.amount) AS total_paid
    //     FROM $invoice_payments_table
    //     WHERE $invoice_payments_table.supplier_id=0 AND $invoice_payments_table.deleted=0 AND $invoice_payments_table.invoice_id=$invoice_id";
    //     $payment = $this->db->query($payment_sql)->getRow();
...
function get_invoice_total_summaryp($invoice_id, $supplier_id) {
    ...
    protected function get_sales_total_metapayable($id, $main_table, $items_table) {
        ...
        $invoice_sql = "SELECT $main_table.id, $main_table.discount_amount, $main_table.discount_amount_type, $main_table.discount_type,
                ...
                LEFT JOIN (SELECT SUM($items_table.supplier_price) AS total_taxable, $items_table.invoice_id,alltotal FROM $items_table WHERE $items_table.deleted=0 AND $items_table.taxable = 1 GROUP BY $items_table.invoice_id) AS taxable_item ON taxable_item.invoice_id = $main_table.id
                LEFT JOIN (SELECT SUM($items_table.supplier_price) AS total_non_taxable, $items_table.invoice_id  FROM $items_table WHERE $items_table.deleted=0 AND $items_table.taxable = 0 GROUP BY $items_table.invoice_id) AS non_taxable_item ON non_taxable_item.invoice_id = $main_table.id
                WHERE $main_table.deleted=0 AND $main_table.id = $id";

        $invoice_info = $this->db->query($invoice_sql)->getRow();

        if (!$invoice_info || $invoice_info->supplier != 1) {
            return null;
        }
    ...
    function invoice_statistics($options = array()) {
        ...
        if ($payments) {
            ...
            WHERE $invoice_payments_table.supplier_id=0 AND $invoice_payments_table.deleted=0 AND YEAR($invoice_payments_table.payment_date)=$year AND $invoices_table.deleted=0 $where $payments_where
...
function get_invoice_total_meta($invoice_id) {
    ...
    function get_invoices_summary($options = array()) {
        ...
        $sql = "SELECT COUNT($invoices_table.id) AS invoice_count, SUM($invoices_table.invoice_total) AS invoice_total, SUM($invoices_table.discount_total) AS discount_total, SUM($invoices_table.tax) AS tax_total, SUM($invoices_table.tax2) AS tax2_total, SUM($invoices_table.tax3) AS tax3_total,
            ...
            FROM $invoices_table
            LEFT JOIN $clients_table ON $clients_table.id = $invoices_table.client_id             
            LEFT JOIN (SELECT SUM($invoice_payments_table.amount) AS payment_received, $invoice_payments_table.invoice_id FROM $invoice_payments_table WHERE $invoice_payments_table.deleted=0 AND $invoice_payments_table.supplier_id=0 GROUP BY $invoice_payments_table.invoice_id) AS payments_table ON payments_table.invoice_id = $invoices_table.id
            WHERE $invoices_table.deleted=0 AND $invoices_table.status = 'not_paid' $where
