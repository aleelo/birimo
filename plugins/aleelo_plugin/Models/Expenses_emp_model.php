...
class Expenses_emp_model extends Crud_model {

    protected $table = null;

    function __construct() {
        $this->table = 'expenses_emp';
        parent::__construct($this->table);
    }

    function get_details($options = array()) {
        $expenses_table = $this->db->prefixTable('expenses_emp');
        $expense_categories_table = $this->db->prefixTable('expense_categories');
        $projects_table = $this->db->prefixTable('projects');
        $users_table = $this->db->prefixTable('users');
        $taxes_table = $this->db->prefixTable('taxes');
        $clients_table = $this->db->prefixTable('clients');
        $expense_payments_table = $this->db->prefixTable('expense_payments_emp');
    ...
    function get_income_expenses_info($options = array()) {
        $expenses_table = $this->db->prefixTable('expenses_emp');
        $invoice_payments_table = $this->db->prefixTable('invoice_payments_emp');
    ...
    function get_summary_details($options = array()): mixed {
        $expenses_table = $this->db->prefixTable('expenses_emp');
        $expense_categories_table = $this->db->prefixTable('expense_categories');
        $taxes_table = $this->db->prefixTable('taxes');
        $where = "";
