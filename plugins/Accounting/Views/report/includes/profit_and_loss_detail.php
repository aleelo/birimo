      ...
      <div class="row">
        <div class="col-md-10">
          ...
          <div class="row">
            ...
            <div class="col-md-3">
              <div class="form-group">
                <label for="invoice_id" class=""><?php echo app_lang('invoice'); ?></label>
                <?php
                $invoice_options = ['' => '- ' . app_lang('invoice') . ' -'];
                if (isset($invoices_dropdown) && is_array($invoices_dropdown)) {
                  foreach ($invoices_dropdown as $key => $value) {
                    $invoice_options[$key] = $value;
                  }
                }
                $selected_invoice = isset($invoice_id) ? $invoice_id : '';
                echo form_dropdown("invoice_id", $invoice_options, array($selected_invoice), "class='select2 validate-hidden' id='invoice_id'");
                ?>
              </div>
            </div>
